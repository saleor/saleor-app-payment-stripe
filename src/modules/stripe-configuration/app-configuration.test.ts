...
import { type PrivateMetadataAppConfigurator } from "../app-configuration/app-configuration";
import { OBFUSCATION_DOTS } from "../app-configuration/utils";
import { type AdyenAppConfig, type ChannelMapping, adyenAppConfigSchema } from "./app-config";
import { type AdyenEntryConfig } from "./stripe-entries-config";
import { AppConfigurator } from "./app-configuration";
import { configEntryRequired, configEntryAll } from "./__tests__/mocks";
...
describe("AppConfigurator", () => {
  ...
  const appConfigurator = new AppConfigurator(metadataManagerMock, saleorApiUrlMock);
  ...
  describe("setConfigEntry", () => {
    it("should update app config with new config entry added to list of config entries", async () => {
      const newConfigEntry: AdyenEntryConfig = {
        ...configEntryRequired,
        configurationId: "new-mock-id",
      };
      const existingConfig: AdyenAppConfig = {
        configurations: [configEntryRequired],
    ...
    it("should update app config with update config entry changed in list of config entries", async () => {
      const updateConfigEntry: AdyenEntryConfig = {
        ...configEntryAll,
        configurationId: "new-mock-id",
      };
      const existingConfig: AdyenAppConfig = {
  ...
  describe("setMapping", () => {
    it("should call setConfig with new mapping", async () => {
      ...
      const existingConfig: AdyenAppConfig = { configurations: [], channelToConfigurationId: {} };
  ...
  describe("setConfig", () => {
    it("should call setConfig on configurator", async () => {
      const newConfig: Partial<AdyenAppConfig> = {
        configurations: [],
        channelToConfigurationId: {},
      };
